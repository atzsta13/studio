...
### 🟨 AGENT 2: The Passport Persistor (Persistence)
**Task**: Make the Gamification/Passport state permanent.
**Instructions**:
1. Create `ui/passport/PassportViewModel.kt`.
2. When a stamp is toggled in the `PassportScreen`, write it to the `UserProgress` entity in Room.
3. On screen load, collect the `UserProgress` flow from `UserDao` to hydrate the progress bar and XP rank.
