...
import { FestivalSwitcher } from './festival-switcher';
...
export default function Header({ festivalId }: { festivalId?: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/10 bg-background/80 backdrop-blur-3xl transition-all duration-500">
      <div className="container flex h-14 md:h-18 items-center justify-between">
        <div className="flex items-center gap-10">
          <Link href={rootHref} className="flex items-center gap-2.5 group">
            ...
            <span className="font-headline font-black text-lg md:text-2xl tracking-tighter uppercase italic">
              {config.name} <span className="text-primary text-glow">Insider</span>
            </span>
          </Link>

          <nav className="hidden md:flex items-center gap-8 text-[11px] font-black uppercase tracking-[0.25em]">
            {navItems.map((item) => {
        ...
        <div className="flex items-center gap-3">
          {festivalId && <FestivalSwitcher festivalId={festivalId} />}
          <ModeToggle />
        </div>
      </div>
    </header>
