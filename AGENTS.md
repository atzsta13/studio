...
1. **This file** — rules, commands, architecture summary.
2. **`TASKS.md`** — the backlog. If you were given a bug or feature, it is probably already specified there with exact file paths, the fix, and a verify step. Follow that spec.
3. Deeper docs only if the task router below points you there.
...
|---|---|
| `public/data/` | `npm run lineup:sync` (source: `festivals/<id>/`) |
| `android/app/src/main/assets/<festival-id>/` | same sync |
| `public/manifest.json` | `scripts/generate-manifest.mjs` (runs on prebuild) |

Edit the source in `festivals/<id>/` and re-run the sync — direct edits to these directories get silently overwritten.

### Definition of done

- Web change: `npm run typecheck` + `npm run lint` + `npm test -- --run` all green.
- Android change: `./gradlew test` green.
- Data change: `npm run lineup:sync` ran, and the synced outputs are committed together with the source.
- You touched none of the "What Was Removed" list below and violated no Hard Constraint.
