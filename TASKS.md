...
- [ ] **AI Scout is a non-functional prototype** **[audited 2026-07-04, re-audited 2026-07-10]** — the prompt layer gets *broader* (grows with every festival) and *more repetitive* (duplicated construction, config ignored). Blockers, in order:
  1. **No retrieval → unbounded prompt.** `LocalScoutRepository.getLocalRecommendationsStreaming()` (`LocalScoutRepository.kt:147`) stuffs the **entire lineup** (458 artists for Sziget ≈ 15–18k tokens) into the prompt while `setMaxTokens(512)` (line 132) is the total context budget → fails/truncates for every festival except the tiniest, and gets worse each time a festival is added. The `// Context pruning (RAG-Lite)` comment on line 155 is a lie — there is no pruning. Fix: pre-filter to ~top-20 candidates (genre/vibe/schedule match) before prompting, raise maxTokens to 2–4k.
  2. **`config.aiPersona` ignored → not config-first + repetitive.** Every festival defines `aiPersona` in `config.json` and it's parsed in `FestivalConfig.kt:157`, but `LocalScoutRepository.kt:161` hardcodes `"You are the high-energy Festival Scout"` — so every festival's Scout sounds identical and there are two sources of truth. Read the persona from config.
  3. **Prompt construction is scattered across 4 sites** — `AcousticRepository.buildSignaturePrompt()` → `DiscoverViewModel.startAcousticScout()` → `DiscoverViewModel.runLocalScout()` (contextPrefix, line 155) → `LocalScoutRepository` (full template). No shared builder; each concatenates strings ad hoc. Consolidate into one prompt builder that takes (persona, filtered candidates w/ schedule, user query).
  4. Model download URL is `config.productionUrl + "/ai/gemma4-2b-android.bin"` — nobody hosts a 1.2GB model at those (dead) URLs, and GitHub Pages can't (100MB limit). Only the adb-push "SCAN LOCAL" path works. Decide hosting or declare it a power-user feature.
  5. Fake streaming: blocking `generateResponse()` then word-by-word replay with 30ms delays (`LocalScoutRepository.kt:174–179`). Use `generateResponseAsync`.
  - The Acoustic Scout (`AcousticRepository.kt`) maps mic RMS + zero-crossings to ~9 canned sentences (3 volume × 3 energy, lines 93–107) — it cannot identify a set, and the candidate context passed alongside it has no schedule/stage/time, so the LLM can't answer "who is playing right now" even though that data now exists. Reframe or feed it the live schedule.
